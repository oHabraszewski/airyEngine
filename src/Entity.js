...
class Entity{
    ...
    move(fps){
        ...
        this.x += Math.floor((this.velocity.x / fps) + (toAddX/fps));
        this.y += Math.floor((this.velocity.y / fps) + (toAddY/fps))
    }
    draw(context = null, camera){
        if(context != null && camera != undefined){
            ...
            context.save()
            context.translate(Math.floor(this.cx + this.pivot.x), Math.floor(this.cy + this.pivot.y))
